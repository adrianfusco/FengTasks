...
class Client:
    ...
    GET_UNTASKS_WITH_FILTERS_PAYLOAD = {
        ...
        'filter' : 'assigned_to',           
        'fval' : '-1',              
        'tasksGroupBy' : 'nothing',         # Group by 'Assigned to'
        'tasksOrderBy' : 'created_on',      # Order 'Created_on'
        'context' : '{"1":[0,71],"2":[0]}',
        'currentdimension' : '1',
        'ajax' : 'true',
        'c' : 'task',
    ...
    def getTasks(self: object):
        ...
        if json_tasks['groups']:
            for task_information in json_tasks['groups'][0]['group_tasks']:
                task_url = self.domain + self.URL_BASE_TASK + str(task_information['id'])
